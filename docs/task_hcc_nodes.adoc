...
.. To add or edit the stored BMC credentials for a compute node with BMC connection errors, select *Edit connection settings* in the error message text in the *BMC Connection Status* column. Only if the connection attempt fails for a compute node, an error message is displayed in this column for that node.
